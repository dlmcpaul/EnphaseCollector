...
class EnphaseServiceTest {
	...
	void CollectionDateTest() {
		...
		// Given
		Mockito.when(this.mockEnphaseService.collectEnphaseData(false)).thenReturn(makeSystem(now));
		Mockito.when(this.mockEnphaseService.getCollectionTime(any(System.class))).thenCallRealMethod();
		// When
		LocalDateTime collectionTime = this.mockEnphaseService.getCollectionTime(this.mockEnphaseService.collectEnphaseData(false).orElseThrow());
		// Then
